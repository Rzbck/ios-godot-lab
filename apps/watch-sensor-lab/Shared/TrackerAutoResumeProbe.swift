...
struct TrackerAutoResumeProbe: Equatable {
    ...
    mutating func observe(
        ...
        // Two fixes separated by a sensor gap are not convergent movement
        // evidence. The later fix starts a new proof sequence.
        if let lastSampleTimestamp, sampleTimestamp - lastSampleTimestamp > 2.5 {
            filteredSpeedMps = 0
            consecutiveReliableMovementSamples = 0
        }
    ...
    /// Compatibility name for deterministic callers. Production additionally
    /// consumes the quality/freshness fields through `resumeDecision`.
    func confirmedRecentSpeedMps(
        now: TimeInterval,
        maxAge: TimeInterval = 2.5
    ) -> Double {
        hasSustainedMovement(now: now, maxAge: maxAge) ? recentSpeedMps(now: now, maxAge: maxAge) : 0
    }
}
