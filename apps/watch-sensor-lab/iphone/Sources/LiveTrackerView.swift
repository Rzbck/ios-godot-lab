...
struct LiveTrackerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            LinearGradient(
                colors: [.black.opacity(0.52), .clear, .black.opacity(0.28)],
                startPoint: .top,
                endPoint: .bottom
            )
            .ignoresSafeArea()
        ...
        .onChange(of: tracker.route.count) { _, _ in
            ...
            withAnimation(.easeOut(duration: 0.35)) {
                cameraPosition = .region(
                    MKCoordinateRegion(
                        center: coordinate,
                        latitudinalMeters: tracker.isActive ? 650 : 1_400,
                        longitudinalMeters: tracker.isActive ? 650 : 1_400
                    )
                )
            }
        }
    ...
    private var liveMap: some View {
        ...
        .simultaneousGesture(
            DragGesture(minimumDistance: 12)
                .onChanged { _ in followUser = false }
        )
    }
    ...
    private var header: some View {
        HStack(spacing: 10) {
            ...
            HStack(spacing: 6) {
                Circle()
                    .fill(tracker.watchReachable ? Color.green : Color.orange)
                    .frame(width: 8, height: 8)
                Image(systemName: "applewatch")
                Text(tracker.watchReachable ? "LIVE" : "WATCH")
    ...
    private var statusPill: some View {
        HStack(spacing: 6) {
            Circle()
                ...
                .frame(width: 8, height: 8)
            Text(tracker.isPaused ? "PAUSE" : "REC")
...
private struct ReadinessChip: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("\(title), \(ready ? "prêt" : "en attente")")
    }
}
